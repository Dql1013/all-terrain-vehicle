...
/*			------------------------------------------------------------
			可以先使用速度闭环并取消循迹高效的测量出两轮在闭环状态下的pwm设置值，读取后单独使用*/	
//		例：
			//Set_PWM(1265,1103);//闭环速度稳定后显示的pwm数据直接输入到设置速度中
		}
		else Tracks_Control(left_speed,right_speed);//闭环测速时计得注释，改行代码开启循迹，并且不会与闭环速度冲突
